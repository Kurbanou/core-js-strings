...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    const shift = (char.charCodeAt(0) - (char < 'a' ? 65 : 97) + 13) % 26;
    return String.fromCharCode(shift + (char < 'a' ? 65 : 97));
  });
}
...
function getCardId(value) {
  ...
  const suitIndex = suits.findIndex((suit) => value.includes(suit));
  const rank = value.replace(suits[suitIndex], '');
